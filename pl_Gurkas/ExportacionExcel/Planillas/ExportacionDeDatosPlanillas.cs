...
namespace pl_Gurkas.ExportacionExcel.Planillas
{
    class ExportacionDeDatosPlanillas
    {
        public void ExportarDatosBajasPlanillasExcel(DataGridView dgView, ProgressBar pBar, string nombre_unidad, string fi, string ff)
        {
            try
            {
                SaveFileDialog fichero = new SaveFileDialog();
                fichero.Filter = "Excel (*.xls)|*.xls";
                fichero.FileName = "Personal de Baja de la unidad " + nombre_unidad + " Fecha " + fi + " al " + ff;
                if (fichero.ShowDialog() == DialogResult.OK)
                {
                    ...
                    foreach (DataGridViewRow fila in dgView.Rows) //Aquí leemos las filas de las columnas leídas
                    {
                        ...
                        foreach (DataGridViewColumn columna in dgView.Columns)
                        {
                            ...
                            formatRange = hoja_trabajo.get_Range("C" + (iFil + 1), "E" + (iFil + 1));
        ...
        public void ExportarDatosPersonalPlanillasExcel(DataGridView dgView, ProgressBar pBar, string nombre_unidad)
        {
            try
            {
                SaveFileDialog fichero = new SaveFileDialog();
                fichero.Filter = "Excel (*.xls)|*.xls";
                fichero.FileName = "Datos laborales de " + nombre_unidad ;
                if (fichero.ShowDialog() == DialogResult.OK)
                {
                    if (pBar != null)
                    {
                    ...
                    foreach (DataGridViewRow fila in dgView.Rows) //Aquí leemos las filas de las columnas leídas
                    {
                        ...
                        foreach (DataGridViewColumn columna in dgView.Columns)
                        {
                            iCol++;
                            hoja_trabajo.Cells[iFil + 1, iCol] = fila.Cells[columna.Name].Value;
                            formatRange = hoja_trabajo.get_Range("A" + (iFil + 1), "O" + (iFil + 1));
                            formatRange.NumberFormat = "@";
                        }
                        pBar.Value += 1;
                    }
